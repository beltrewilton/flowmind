  ...
  def render(assigns) do
    ...
    <div id={"dropdown-#{@chat.id}"}>
      <.dropdown align="end" class="absolute cursor-pointer text-gray-400 top-0 right-0">
        ...
        <.menu
          tabindex="0"
          class="max-w-20 dropdown-content bg-primary text-primary-content rounded-box z-[1] w-52 p-2 shadow"
        >
          <:item>
            <label id={@chat.id} phx-click="handle_repply" phx-value-whatsapp_id={@chat.whatsapp_id}>
              reply
            </label>
          </:item>
        </.menu>
