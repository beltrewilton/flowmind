  ...
  def render(assigns) do
    ...
    <div>
      ...
      <div :if={@chat.message_type == "application"}>
        <canvas id="the-canvas-2" phx-hook="PdfPreview" data-docurl={@chat.message} data-scale={0.2}>
        </canvas>
        <.icon name="hero-document-text" /> {Path.basename(@chat.message)}
      </div>
      <div :if={@chat.message_type == "image"}>
        <img src={@chat.message} alt="" class="w-24 h-24 object-cover rounded-md shadow-sm" />
      </div>
      ...
      <div :if={@chat.message_type == "video"}>
        <video
          ...
          <video
          controls
          class="w-32 h-20 md:w-40 md:h-24 lg:w-48 lg:h-28 rounded-md shadow-sm object-cover"
        >
